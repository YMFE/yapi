...
module.exports = function(jsondiffpatch, formattersHtml, curDiffData) {
  ...
  if (curDiffData && curDiffData.current) {
    const { current, old, diffType } = curDiffData
    if (diffType === 'mock') {
      if (current.enable !== old.enable) {
        diffView.push({
          title: '启用状态',
          content: diffText(old.enable, current.enable)
        })
      }
      if (current.mock_script !== old.mock_script) {
        diffView.push({
          title: 'mock 脚本',
          content: diffText(old.mock_script, current.mock_script)
        })
      }
      if (current.uid !== old.uid) {
        diffView.push({
          title: '变更人',
          content: diffText(old.uid, current.uid)
        })
      }
    }
  }
