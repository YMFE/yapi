...
class InterfaceColContent extends Component {
  ...
  async handleColIdChange(newColId) {
    ...
    await this.props.fetchCaseList(newColId).then(res => {
      this.setState({
        colLoading: false,
      })
    })
  ...
  handleColdata = (rows, currColEnvObj = {}) => {
    ...
    this.setState({ rows: newRows, colLoading: false })
  }
        ...
        cell: {
          formatters: [
            (text, { rowData }) => {
              ...
              if (record.interface_type === 'dubbo') {
                path = `方法:${record.r_method}; 接口:${record.r_facade}`
              }
              return (
            ...
            {this.state.hasPlugin ? (
              ...
              >
                {this.props.curProjectRole !== 'guest' && (
                  <Tooltip title="在 落兵台 服务端跑自动化测试，测试环境不能为私有网络，请确保 落兵台 服务器可以访问到自动化测试环境domain">
                    <Button
                      style={{
                        marginRight: '8px',
                      }}
              ...
              <tr align="center">
                <td colSpan="5">
                  <Spin
                    ...
                    size="large"
                    spinning={this.state.colLoading}
                  />
                </td>
              </tr>
            </tfoot>
