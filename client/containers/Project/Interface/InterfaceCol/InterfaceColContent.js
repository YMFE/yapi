...
class InterfaceColContent extends Component {
  ...
  handleColdata = (rows, currColEnv = '', project_id = null) => {
    ...
    //   return item;
    // })
    // const { currColEnv, curProjectId } = this.state;
    // if (curProjectId && currColEnv) {
    // }
    ...
    let newRows = produce(rows, draftRows => {
      draftRows.map(item => {
        ...
        item.id = item._id;
        item._test_status = item.test_status;
        item.case_env = item.project_id === project_id ? currColEnv || item.case_env : item.case_env;
    ...
    return (
      <div className="interface-col">
        <h2
          ...
          style={{ display: 'inline-block', margin: '0 20px', marginBottom: '16px' }}
        >
          测试集合&nbsp;<a
            target="_blank"
        ...
        {this.state.hasPlugin ? (
          <div style={{ float: 'right' }}>
            {this.props.curProjectRole !== 'guest' && (
              <Tooltip title="在 YApi 服务端跑自动化测试，测试环境不能为私有网络，请确保 YApi 服务器可以访问到自动化测试环境domain">
                <Button style={{ marginRight: '8px' }} onClick={this.autoTests}>
                  服务端测试
                </Button>
              </Tooltip>
            )}
          ...
          <Tooltip title="请安装 cross-request Chrome 插件">
            <Button disabled type="primary" style={{ float: 'right' }}>
              开始测试
            </Button>
          </Tooltip>
        )}

        <CaseEnv envList={this.props.envList} currProjectEnvChange={this.currProjectEnvChange} />
