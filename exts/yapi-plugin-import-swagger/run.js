...
const parseSwagger = node => {
  let res
  switch (node.type) {
    case 'object':
      res = {}
      if (node.properties) {
        Object.keys(node.properties).forEach(v => {
          res[v] = parseSwagger(node.properties[v])
        })
      }
      break
    case 'array':
...
async function run(res) {
  ...
  isOAS3 = res.openapi && /^3\./.test(res.openapi)
  if (isOAS3) {
...
function handleSwagger(data) {
  ...
  // adv mock swagger 真实数据
  api.adv_mock_case = {}

  try {
    let resBodyJSON = JSON.parse(api.res_body)
    // adv mock swagger 真实数据
    let advMock = Object.create(null)
    advMock = parseSwagger(resBodyJSON)
    if (Object.keys(advMock).length !== 0) {
      api.adv_mock_case = advMock
    } else {
      delete api.adv_mock_case
    }
    api.res_body_type = 'json'
    api.res_body_is_json_schema = true
  } catch (e) {
