...
function connect(callback) {
  ...
  if (config.db.connectString) {
    ...
    if (config.db.replset) {
      options['replset'] = { rs_name: config.db.replset }
      connectString = 'mongodb://'
      config.db.servername.forEach((element, index) => {
        connectString += `${element}:${config.db.port}${
          index !== config.db.servername.length - 1 ? ',' : ''
        }`
      })
      connectString += `/${config.db.DATABASE}`
    } else {
