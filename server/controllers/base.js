...
class baseController {
  ...
  async init(ctx) {
    ...
    let ignoreRouter = [
      ...
      '/api/user/reg',
      '/api/user/status',
      '/api/user/logout', // 接入单点登出后不使用
      '/api/user/avatar',
      '/api/user/login_by_ldap',
      // 公开 doc 目录
      '/api/plugin/wiki_action/get_detail',
    ]
    if (ignoreRouter.indexOf(ctx.path) > -1 || /^\/api\/open/.test(ctx.path)) {
      this.$auth = true
    } else {
  ...
  async getUidByUserName(userName) {
    let userInst = yapi.getInst(userModel) //创建user实体
    let result = await userInst.findByEmail(`${userName}`)
  ...
  async getProjectRole(id, type, uid) {
    let result = {}
    try {
      if (!uid && this.getRole() === 'admin') {
        return 'admin'
      }
      if (type === 'interface') {
      ...
      if (type === 'group') {
        let userId = (uid && parseInt(uid, 10)) || this.getUid()
        let groupInst = yapi.getInst(groupModel)
        let groupData = await groupInst.get(id)
        // 建立分组的人
        if (groupData.uid === userId) {
          return 'owner'
        }
   ...
   */
  async checkAuth(id, type, action, userid) {
    let role = await this.getProjectRole(id, type, userid)
