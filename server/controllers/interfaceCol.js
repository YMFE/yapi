...
class interfaceColController extends baseController {
  ...
  async list(ctx) {
    try {
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        let caseList = await this.caseModel.listByParent('', result[i]._id)

        for (let j = 0; j < caseList.length; j++) {
          let item = caseList[j].toObject()
          item['caseList'] = await this.getDescendants(item['_id'])
          if (item.record_type === 2) {
            caseList[j] = item
          } else {
  ...
  async getCaseEnvList(ctx) {
    try {
      ...
      for (let i = 0; i < projectList.length; i++) {
        let result = await this.projectModel.getBaseInfo(
          projectList[i],
          'name  env',
        )
  ...
  async addCase(ctx) {
    try {
      ...
      params.interface_type = params.interface_type || 'http'
      params.record_type = params.record_type || 0
      if (params.parent_id) {
        let parentData = await this.caseModel.get(params.parent_id)
        params.ancestors = `${parentData.ancestors},${parentData._id}`
      } else {
        params.ancestors = ''
      }
  ...
  async addCaseList(ctx) {
    try {
      ...
      if (params.parent_id) {
        let parent = await this.caseModel.get(params.parent_id)
        if (parent) {
          data.ancestors = `${parent.ancestors},${params.parent_id}`
        } else {
          data.parent_id = ''
          data.ancestors = ''
        }
      ...
      for (let i = 0; i < params.interface_list.length; i++) {
        ...
        if (
          ...
          let req_body_other = yapi.commons.json_parse(
            interfaceData.req_body_other,
          )
      ...
      this.colModel.get(caseData.col_id).then(col => {
        yapi.commons.saveLog({
          content: `<a href="/user/profile/${this.getUid()}">${username}</a> 在接口集 <a href="/project/${
            ...
            caseData.project_id
          }/interface/case/${params.id}">${params.casename ||
            caseData.casename}</a>`,
          type: 'project',
          uid: this.getUid(),
          username: username,
  ...
  async upDir(ctx) {
    try {
      ...
      let upParams = {}
      if (params.col_id && params.col_id !== caseData.col_id) {
        upParams['col_id'] = params.col_id
      }
      ...
      let result = await this.caseModel.up(params.id, upParams).then(
        res => {},
        err => {
      ...
      if (caseData.record_type === 2) {
        // 目录节点，需要更改子孙节点的 col_id、ancestors 数据
        let desUpParams = {}
        if (params.col_id && params.col_id !== caseData.col_id) {
          desUpParams['col_id'] = params.col_id
        }
        let descendants = await this.caseModel.listByAncestor(params.id)
        descendants.forEach(item => {
          let newAncestors = `${upParams.ancestors},${params.id}`
          let oldAncestors = `${caseData.ancestors},${params.id}`
          desUpParams['ancestors'] = item.ancestors.replace(
            oldAncestors,
            newAncestors,
          )
          this.caseModel.up(item._id, desUpParams).then(
            res => {},
            err => {
              yapi.commons.log(err.message, 'error')
            },
      ...
      yapi.commons.saveLog({
        content: `<a href="/user/profile/${this.getUid()}">${username}</a> 拖动了 <a href="/project/${
          dragNode.project_id
        }/interface/col/${dragNode.col_id}">${dragNode.casename}</a>`,
        type: 'project',
        uid: this.getUid(),
        username: username,
  ...
  async getCase(ctx) {
    try {
      ...
      result.req_headers = yapi.commons.handleParamsValue(
        data.req_headers,
        result.req_headers,
      )
      result.res_body = data.res_body
      result.res_body_type = data.res_body_type
      result.interface_type = data.interface_type
      result.r_facade = data.r_facade
      result.r_method = data.r_method
      result.req_body_form = yapi.commons.handleParamsValue(
        data.req_body_form,
        result.req_body_form,
      )
      result.req_query = yapi.commons.handleParamsValue(
        data.req_query,
        result.req_query,
      )
      result.req_params = yapi.commons.handleParamsValue(
        data.req_params,
        result.req_params,
      )
  ...
  async upColIndex(ctx) {
    try {
      ...
      yapi.commons.saveLog({
        content: `<a href="/user/profile/${this.getUid()}">${username}</a> 拖动了 <a href="/project/${
          dragNode.project_id
        }/interface/col/${dragNode._id}">${dragNode.name}</a>`,
        type: 'project',
        uid: this.getUid(),
        username: username,
        typeid: dragNode.project_id,
  ...
  async delCase(ctx) {
    try {
      ...
      // 删除子孙节点
      await this.caseModel.delByAncestor(caseid)
      let username = this.getUsername()
      this.colModel.get(caseData.col_id).then(col => {
        yapi.commons.saveLog({
          content: `<a href="/user/profile/${this.getUid()}">${username}</a> 删除了接口集 <a href="/project/${
            caseData.project_id
          }/interface/col/${caseData.col_id}">${col.name}</a> 下的 ${
            caseData.casename
          }`,
          type: 'project',
          uid: this.getUid(),
