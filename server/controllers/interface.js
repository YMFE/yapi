...
class interfaceController extends baseController {
  ...
  async getDescendants(parentId, filter) {
    ...
    let children
    if (filter) {
      children = await this.Model.list({
        parent_id: parentId,
        record_type: {
          $in: [0, 2],
        },
      ...
      children = await this.Model.listByParentId(parentId)
    }
   ...
   * @param {String}   record_type 接口类型描述
  ...
  async add(ctx) {
    ...
    if (params.desc && !params.markdown) {
      params.markdown = params.desc
      params.desc = mdRender.render(params.markdown)
    }
    if (params.markdown && !params.desc) {
      params.desc = mdRender.render(params.markdown)
    }
    ...
    switch (params.record_type) {
      case 0:
        if (params.interface_type === 'http') {
          checkRepeat = await this.Model.checkRepeat({
          ...
          checkRepeat = await this.Model.checkRepeat({
            r_facade: params.r_facade,
            r_method: params.r_method,
          })
        }
    ...
    if (checkRepeat > 0) {
      ...
      switch (params.record_type) {
        case 0:
          if (params.interface_type === 'http') {
            msg = '已存在的接口:' + params.path + '[' + params.method + ']'
          } else {
            msg = '该接口已存在'
          }
          break
        case 1:
          msg = `当前目录下存在同名${textMap[params.record_type]}`
          break
        case 2:
          msg = '该目录已存在'
          break
      }
      if (params.record_type === 2 && params.from === 'import') {
        let resList = await this.Model.list({
          project_id: params.project_id,
          parent_id: params.parent_id,
          title: params.title,
          catid: params.catid,
        ...
        return (ctx.body = yapi.commons.resReturn(resList[0]))
      } else {
        return (ctx.body = yapi.commons.resReturn(null, 40022, msg))
      }
    }
    ...
    if (params.parent_id !== '') {
      let parentItem = await this.Model.get(params.parent_id)
      if (parentItem && parentItem._id) {
        data.ancestors = `${parentItem.ancestors},${params.parent_id}`
      } else {
        data.ancestors = ''
      }
    } else {
    ...
    this.catModel.get(params.catid).then(cate => {
      ...
      let title = `<a href="/user/profile/${this.getUid()}">${username}</a> 为分类 <a href="/project/${
        params.project_id
      }/interface/api/cat_${params.catid}">${cate.name}</a> 添加了${
        textMap[data.record_type]
      } `
      if (data.record_type === 2) {
        title += `${data.title}`
      } else {
        title += `<a href="/project/${params.project_id}/interface/api/${result._id}">${data.title}</a> `
      }
      let logData = {
        interface_id: result._id,
        cat_id: data.catid,
        type: 'project',
        uid: this.getUid(),
        username: username,
  ...
  async save(ctx) {
    ...
    let result = await this.Model.getIdByProIdCatId(
    ...
    if (result.length > 0) {
      ...
      if (validResult.valid) {
        let data = {}
        data.params = params
        let addResult = await this.add(data)
        return (ctx.body = yapi.commons.resReturn([addResult])) //这个地方用数组是为了和更新一致
      } else {
        return (ctx.body = yapi.commons.resReturn(
          null,
          400,
  ...
  async list(ctx) {
    ...
    try {
      ...
      if (limit === 'all') {
        ...
        result = await this.Model.listWithPage(
          {
            project_id: project_id,
            record_type: 0,
          },
      ...
      let count = await this.Model.listCount({ project_id, record_type: 0 })
  ...
  async listByCat(ctx) {
    ...
    try {
      ...
      let result = await this.Model.listByCatidWithPage(
        {
          catid: catid,
          record_type: 0,
        },
   ...
   * @returns
   * @memberof interfaceController
   * @example ./api/interface/list_menu
   */
      ...
      for (let i = 0, item, list; i < result.length; i++) {
        ...
        for (let j = 0; j < list.length; j++) {
          ...
          if (list[j]['record_type'] === 2) {
            list[j]['list'] = await this.getDescendants(list[j]['_id'], filter)
          }
        }
        item.itemType = 'cat'
        item.list = list
  ...
  async up(ctx) {
    try {
      ...
      if (!interfaceData) {
        return (ctx.body = yapi.commons.resReturn(null, 400, '该内容不存在'))
      }
      if (!this.$tokenAuth) {
      ...
      switch (params.record_type) {
        case 0:
          if (params.interface_type === 'http') {
            ...
            if (
              (params.r_facade && params.r_facade !== interfaceData.r_facade) ||
              (params.r_method && params.r_method !== interfaceData.r_method)
            ) {
              checkRepeat = await this.Model.checkRepeat({
                project_id: params.project_id,
                r_facade: params.r_facade,
          ...
          break
        default:
          if (params.title && params.title !== interfaceData.title) {
            checkRepeat = await this.Model.checkRepeat({
              project_id: params.project_id,
              parent_id: params.parent_id,
              title: params.title,
        ...
        switch (params.record_type) {
          case 0:
            if (params.interface_type === 'http') {
              ...
              msg = `已存在的接口: ${params.r_facade} ${params.r_method} `
            }
            break
          case 1:
            msg = `当前目录下存在同名${textMap[params.record_type]}`
      ...
      this.catModel.get(interfaceData.catid).then(cate => {
        ...
        yapi.commons.saveLog({
          content: `<a href="/user/profile/${this.getUid()}">${username}</a> 
                      ...
                      下的${
                        textMap[interfaceData.record_type]
                      } <a href="/project/${
            cate.project_id
  ...
  /**
   * 查询接口，根据 project_id 和 cat_id 查询其下所有数据，包括 目录信息 + 接口信息
   * 目前只是获取了 _id path method，要获取其他信息可继续添加
   */
  async getAllByCatId(ctx) {
    let projectId = ctx.request.query.project_id
    let catId = ctx.request.query.cat_id
    ...
    try {
      let result = await this.Model.getAllByProIdCatId(
        Number(projectId),
        catId,
        'path method',
      )

      // let project = await this.projectModel.getBaseInfo(projectId);
      // if (project.project_type === 'private') {
      //   if ((await this.checkAuth(project._id, 'project', 'view')) !== true) {
      //     return (ctx.body = yapi.commons.resReturn(null, 406, '没有权限'));
      //   }
  ...
  async del(ctx) {
    try {
      ...
      if (data.record_type === 2) {
        let childList = await this.Model.listByAncestor(id)
        childList.forEach(async item => {
          try {
      ...
      this.catModel.get(data.catid).then(cate => {
        yapi.commons.saveLog({
          content: `<a href="/user/profile/${this.getUid()}">${username}</a> 删除了分类 <a href="/project/${
            cate.project_id
          }/interface/api/cat_${data.catid}">${cate.name}</a> 下的${
            textMap[data.record_type]
          } "${data.title}"`,
          type: 'project',
          uid: this.getUid(),
  ...
  async solveConflict(ctx) {
    try {
      ...
      if (result.edit_uid !== 0 && result.edit_uid !== this.getUid()) {
        userinfo = await this.userModel.findById(result.edit_uid)
        data = {
  ...
  async upDir(ctx) {
    try {
      ...
      let upParams = {}
      if (params.catid && params.catid !== interfaceData.catid) {
        upParams['catid'] = params.catid
      }
      if (params.parent_id === '' || params.parent_id === null) {
        upParams['parent_id'] = ''
        upParams['ancestors'] = ''
      } else {
        let parentData = await this.Model.get(params.parent_id)
        upParams['parent_id'] = `${params.parent_id}`
        upParams['ancestors'] = `${parentData.ancestors},${parentData._id}`
      }
      this.Model.up(id, upParams).then(
        res => {},
        err => {
          yapi.commons.log(err.message, 'error')
        },
      )
      // 目录节点，需要更改子孙节点的 catid、ancestors 数据
      if (interfaceData.record_type === 2) {
        let desUpParams = {}
        if (params.catid && params.catid !== interfaceData.catid) {
          desUpParams['catid'] = params.catid
        }
        let descendants = await this.Model.listByAncestor(id)
        descendants.forEach(item => {
          let newAncestors = `${upParams.ancestors},${id}`
          let oldAncestors = `${interfaceData.ancestors},${id}`
          desUpParams['ancestors'] = item.ancestors.replace(
            oldAncestors,
            newAncestors,
          )
          this.Model.up(item._id, desUpParams).then(
            res => {},
            err => {
              yapi.commons.log(err.message, 'error')
            },
      ...
      let dragNode = ctx.request.body.dragNode
      let username = this.getUsername()
      let cate = await this.catModel.get(dragNode.catid)
      if (!cate) {
        return
      }

      yapi.commons.saveLog({
        content: `<a href="/user/profile/${this.getUid()}">${username}</a> 拖动了 <a href="/project/${
          cate.project_id
        }/interface/api/cat_${dragNode.catid}">${
          cate.name
        }</a> 下的 <a> ${dragNode.name || dragNode.title}</a>`,
        type: 'project',
        uid: this.getUid(),
        username: username,
        typeid: cate.project_id,
  ...
  async upIndex(ctx) {
    try {
      let params = ctx.request.body.changes
  ...
  async upCatIndex(ctx) {
    try {
      ...
      yapi.commons.saveLog({
        content: `<a href="/user/profile/${this.getUid()}">${username}</a> 拖动 <a href="/project/${
          dragNode.project_id
        }/interface/api/cat_${dragNode._id}">${dragNode.name}</a> 目录`,
        type: 'project',
        uid: this.getUid(),
        username: username,
        typeid: dragNode.project_id,
      })
      return (ctx.body = yapi.commons.resReturn('成功！'))
    } catch (e) {
      ctx.body = yapi.commons.resReturn(null, 400, e.message)
  ...
  async invokeDubbo(ctx) {
    let options = ctx.request.body
    let baseUrl = '127.0.0.1:8650'
    let res
